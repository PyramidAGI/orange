...
## Quarks as a lingua franca between sensors and pyramids

A simple robot connects a set of quarks — generated by `grounding.py` from live sensor readings — to a pyramid (a pre-built triangle cluster). For example, a tree climbing robot encounters a branch that is blocking its path:

```
grounding.py reads sensors
  branch_integrity = low    ->  stat rough
  path_density = high       ->  problem
  motor_temp = normal       ->  (no quark)
  grip_force = ok           ->  (no quark)

tick emits: {nature, support, machine, tool}
         -> pyramid282 fires
```
...
Source: `triangles/triangle_natural_anchor1.csv`
